...
float "my_rand"(int %idum)
begin
bb1:					;[#uses=1]
	%reg124 = xor int %idum, 123459876		; <int>	[#uses=4]
	%cast110 = cast int %reg124 to long		; <long>	[#uses=1]
	%reg111 = mul long %cast110, -2092037281		; <long>	[#uses=1]
	%cast1002 = cast long %reg111 to ulong		; <ulong>	[#uses=1]
	%reg112 = shr ulong %cast1002, ubyte 32		; <ulong>	[#uses=1]
	%cast1001 = cast ulong %reg112 to int		; <int>	[#uses=1]
	%reg113 = add int %reg124, %cast1001		; <int>	[#uses=1]
	%reg114 = shr int %reg113, ubyte 16		; <int>	[#uses=1]
	%reg115 = shr int %reg124, ubyte 31		; <int>	[#uses=1]
	%reg108 = sub int %reg114, %reg115		; <int>	[#uses=2]
	%reg116 = mul int %reg108, 127773		; <int>	[#uses=1]
	%reg117 = sub int %reg124, %reg116		; <int>	[#uses=1]
	%reg118 = mul int %reg117, 16807		; <int>	[#uses=1]
	%reg119 = mul int %reg108, 2836		; <int>	[#uses=1]
	%reg125 = sub int %reg118, %reg119		; <int>	[#uses=1]
	%reg126 = xor int %reg125, 123459876		; <int>	[#uses=3]
	%cond1000 = setge int %reg126, 0		; <bool>	[#uses=1]
	br bool %cond1000, label %bb3, label %bb2

bb2:					;[#uses=2]
	%reg127 = add int %reg126, 2147483647		; <int>	[#uses=1]
	br label %bb3

bb3:					;[#uses=2]
	%reg128 = phi int [ %reg127, %bb2 ], [ %reg126, %bb1 ]		; <int>	[#uses=1]
	%cast120 = cast int %reg128 to double		; <double>	[#uses=1]
	%reg122 = mul double %cast120, 0.000000		; <double>	[#uses=1]
	%cast109 = cast double %reg122 to float		; <float>	[#uses=1]
	ret float %cast109

bb4:					;[#uses=0]
	%cast1005 = cast ulong 42 to float		; <float>	[#uses=1]
	ret float %cast1005
end
