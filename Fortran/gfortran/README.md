...
>[!IMPORTANT]
> Please read the [notes for maintainers](#Notes-for-developers-and-maintainers)
> before making any changes to this test suite.
...
The tests in this directory (and subdirectories) must *not* be modified. Tests
may not be added or removed. Only changes to the following files and directories
are permitted:

  - `gfortran/README.md`
  - `gfortran/compile-save-diags.cmake`
  - `gfortran/utils/`

In addition, all `CMakeLists.txt` and `override.yaml` files may be modified.
`tests.cmake` is generated by `utils/update-test-config.py` and must only be
modified by running that script (more information about this is
[here](#Overriding-DejaGNU-Annotations)).
