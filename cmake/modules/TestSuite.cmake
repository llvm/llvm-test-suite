...
function(llvm_test_executable_no_test target)
  cmake_parse_arguments(_LTEARGS "NO_PGO" "" "" ${ARGN})
  add_executable(${target} ${_LTEARGS_UNPARSED_ARGUMENTS})
  if(SMALL_PROBLEM_SIZE)
    append_target_flags(COMPILE_FLAGS ${target} -DSMALL_PROBLEM_SIZE)
  endif()
  ...
  if(TEST_SUITE_PROFILE_USE AND NOT _LTEARGS_NO_PGO)
    if("${TEST_SUITE_PROFILE_DIR}" STREQUAL "")
      ...
      file(RELATIVE_PATH target_reldir "${CMAKE_BINARY_DIR}" "${CMAKE_CURRENT_BINARY_DIR}")
      set(profdata_file "${TEST_SUITE_PROFILE_DIR}/${target_reldir}/${target}.profdata")
      if(NOT EXISTS "${profdata_file}")
        message(FATAL_ERROR "No profile found for ${target}: ${profdata_file}")
      endif()
      append_target_flags(COMPILE_FLAGS ${target} -fprofile-instr-use=${profdata_file})
      append_target_flags(LINK_LIBRARIES ${target} -fprofile-instr-use=${profdata_file})
      get_target_property(_profdata_srcs ${target} SOURCES)
      foreach(_src ${_profdata_srcs})
        set_property(SOURCE ${_src} APPEND PROPERTY OBJECT_DEPENDS "${profdata_file}")
      endforeach()
    endif()
  endif()
  if(TEST_SUITE_SAMPLE_PROFILE_USE AND NOT _LTEARGS_NO_PGO)
